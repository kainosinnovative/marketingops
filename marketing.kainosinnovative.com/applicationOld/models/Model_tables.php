...
class Model_tables extends CI_Model
{
	...
	function search_productCategory($title){
        $this->db->like('category_name', $title , 'both');
	...
	function search_productname($title,$id){
        $this->db->like('product_name', $title , 'both');
        $this->db->order_by('product_name', 'ASC');
		// $this->db->limit(10);
		//$id = $this->session->userdata('cityidsessPopup');
		
		// return $this->db->get('locality_list')->result();
		$query = $this->db->get_where('producttype', array('productcategory_id' => $id ))->result();
		return $query;
    }
}